...
<head>
	...
	<link rel="stylesheet" type="text/css" href="css/css_in-answer.css" />
...
<button class="btn btn-default btn-sm" style="right: 10px;" onclick="window.open('error_send.php');">
报告问题
</button>
&nbsp;
...
foreach ($rows_sel as $key => $v) {
	echo <<<EOT
	<div class="questioncontianer" style="background-image: url('img/graylogo.png'); display: none;" id="que{$i}" queid="{$v['queid']}">
...
?>
			...
			<div class="questioncontrol">
				<div class="quesctrlinside">
					...
					<br>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<script>var toprint = '<button class="btn btn-default quebtn" id="rbtn';
...
for(i = parseInt($('#selcount').attr('v')) + 1; i != parseInt($('#selcount').attr('v')) + parseInt($('#tfcount').attr('v')) + 1; i++) {
	...
	document.write(t);
					...
					<div style="text-align: right;">
						...
						<button class="btn btn-primary btn-lg" style="margin: 80px 30px auto auto;" onclick="onpapersubmitset();">
						交&nbsp;&nbsp;&nbsp;卷
						</button>
					</div>
				</div>
